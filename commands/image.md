...
**批次处理流程**：
- 如果提示词总数 ≤ 10：一次处理完成
- 如果提示词总数 > 10：分多批处理，每批 10 个（最后一批可能不足 10 个）
...
### Step 4: 为每个缺失文件生成单张图片 prompt

对于 `missing_files` 中的每个文件：

1. **提取画风**：从原始 `prompt` 中提取画风描述（如 "digital drawing, stick figure style, beige background, 16:9 aspect ratio..."）

2. **定位图片描述**：根据缺失文件名确定是第几张图片
   - `image_012.png` → 单图场景
   - `image_012_03.png` → 第3张图片，找 "third image shows..."
...
### Step 5: 使用 output_indexes 批量补充生成
...
**output_indexes 计算规则**：
- 单图场景 `image_027.png` → `27`
- 多图场景第N张 `image_012_03.png` → `12 * 100 + 3 = 1203`

### Step 6: 循环验证直到完整

```
🔄 第 1 次验证: 发现 5 个缺失，正在补充生成...
   ✅ 生成完成: 5 张图片
...
脚本会自动：
1. 读取 `script_output.json`，获取每个提示词的 `image_count`
2. 根据命名规则推断预期的图片文件名
3. 扫描 `images/` 目录，对比检查
4. 输出缺失的图片 index 和对应的 prompt
5. **自动生成 `missing_images.json` 文件**（如有缺失）

#### 命名规则

- **单图像** (image_count=1): `image_003.png`（无后缀）
- **多图像** (image_count>1): `image_001_01.png`, `image_001_02.png`, ...

#### 如果发现缺失图片

脚本会输出缺失信息并自动保存 `missing_images.json`：
...
1. 从 `original_prompt` 中提取**画风描述**（如 "digital drawing, stick figure style, beige background, 16:9 aspect ratio..."）
2. 根据缺失文件的位置（如 `_03` 表示第3张）在原 prompt 中找到对应的**图片描述**（如 "third image shows Z"）
3. 合并画风 + 单张图片描述，生成新的单张图片 prompt
...
**output_indexes 命名规则**：
- 单图场景：直接使用 index（如 `27` → `image_027.png`）
- 多图场景某一张：使用 `index * 100 + position`（如 index=12 的第3张 → `1203` → `image_012_03.png`）

**注意**：
- 必须为每个缺失文件生成独立的单张图片 prompt
- `output_indexes` 数组长度必须与 `prompts` 数组长度一致
- 生成单张图片 prompt 时，需要 LLM 理解原 prompt 结构并提取对应内容

#### 循环检查直到完整

```
🔄 第 1 次检查: 发现 2 个缺失，正在补充...
🔄 第 2 次检查: 发现 0 个缺失
✅ 所有图片已生成完毕！
```
...
2. **正确划分批次**：每批最多 10 个提示词
3. **分批调用** MCP prompt_to_image_batch 工具，使用正确的 `start_index`
4. 生成图像文件到 `<project_folder>/images/` 目录
5. **验证图片完整性**：检查所有预期图片是否都已生成
6. **补充生成缺失图片**：如有遗漏，必须补充生成直到完整
7. 创建 `image_metadata.json` 元数据文件，包含：
...
- **🚫 批次大小限制**：每次调用最多 10 个提示词，这是硬性要求
- **🔍 必须验证完整性**：所有批次完成后，必须检查是否有遗漏的图片并补充生成
